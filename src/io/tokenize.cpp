...
ReservedWord GLOBAL_reservedWords[LAST_TOKEN] = {
    {AGATE_TOKEN, "agate"},
    {ALL_TOKEN, "all"},
    {ALPHA_TOKEN, "alpha"},
...
Get_Token ()
{
    ...
    while (GLOBAL_token.Token_Id == END_OF_FILE_TOKEN) {
        ...
        if (c == EOF) {
            ...
            fclose(GLOBAL_dataFile->File); /* added to fix open file buildup JLN 12/91 */
        ...
        case '{' : Write_Token (LEFT_CURLY_TOKEN, GLOBAL_dataFile);
            /* Parse_Comments(GLOBAL_dataFile); */
            break;

        case '}' : Write_Token (RIGHT_CURLY_TOKEN, GLOBAL_dataFile);
            break;                                                

        case '@' : Write_Token (AT_TOKEN, GLOBAL_dataFile);
            break;

        case '&' : Write_Token (AMPERSAND_TOKEN, GLOBAL_dataFile);
            break;

        case '`' : Write_Token (BACK_QUOTE_TOKEN, GLOBAL_dataFile);
            break;

        case '\\': Write_Token (BACK_SLASH_TOKEN, GLOBAL_dataFile);
            break;

        case '|' : Write_Token (BAR_TOKEN, GLOBAL_dataFile);
            break;

        case ':' : Write_Token (COLON_TOKEN, GLOBAL_dataFile);
            break;

        case ',' : Write_Token (COMMA_TOKEN, GLOBAL_dataFile);
            break;

        case '-' : Write_Token (DASH_TOKEN, GLOBAL_dataFile);
            break;

        case '$' : Write_Token (DOLLAR_TOKEN, GLOBAL_dataFile);
            break;

        case '=' : Write_Token (EQUALS_TOKEN, GLOBAL_dataFile);
            break;

        case '!' : Write_Token (EXCLAMATION_TOKEN, GLOBAL_dataFile);
            break;
        ...
        case '^' : Write_Token (HAT_TOKEN, GLOBAL_dataFile);
            break;

        case '<' : Write_Token (LEFT_ANGLE_TOKEN, GLOBAL_dataFile);
            break;

        case '(' : Write_Token (LEFT_PAREN_TOKEN, GLOBAL_dataFile);
            break;

        case '[' : Write_Token (LEFT_SQUARE_TOKEN, GLOBAL_dataFile);
            break;

        case '%' :  Write_Token (PERCENT_TOKEN, GLOBAL_dataFile);
            break;

        case '+' : Write_Token (PLUS_TOKEN, GLOBAL_dataFile);
            break;

        case '?' : Write_Token (QUESTION_TOKEN, GLOBAL_dataFile);
            break;

        case '>' : Write_Token (RIGHT_ANGLE_TOKEN, GLOBAL_dataFile);
            break;

        case ')' : Write_Token (RIGHT_PAREN_TOKEN, GLOBAL_dataFile);
            break;

        case ']' : Write_Token (RIGHT_SQUARE_TOKEN, GLOBAL_dataFile);
            break;
        ...
        case '\'': Write_Token (SINGLE_QUOTE_TOKEN, GLOBAL_dataFile);
            break;

            /* enable C++ style commenting */
        case '/' :          c2 = getc(GLOBAL_dataFile->File);
            if(c2 != (int) '/' && c2 != (int) '*'){
        ...
        case '*' : Write_Token (STAR_TOKEN, GLOBAL_dataFile);
            break;

        case '~' : Write_Token (TILDE_TOKEN, GLOBAL_dataFile);
            break;

        case '"' : Parse_String (GLOBAL_dataFile);
            break;
        ...
        default:
            fprintf (stderr, "Error in %s line %d\n", GLOBAL_dataFile->Filename, GLOBAL_dataFile->Line_Number+1);
        ...
        if (GLOBAL_token.Token_Id == INCLUDE_TOKEN)
        {
            if (Skip_Spaces (GLOBAL_dataFile) != TRUE)
                Token_Error (GLOBAL_dataFile, "Expecting a string after INCLUDE\n");
            ...
            GLOBAL_dataFile->Filename = new char[strlen(GLOBAL_token.Token_String) + 1];
            ...
            if ((GLOBAL_dataFile->File = Locate_File (GLOBAL_token.Token_String, "r")) == NULL) {
...
case 1: if (isdigit(c))
    ...
    {Stuff_Character(c, GLOBAL_dataFile); Phase = 2;
    }
    else
        if ((c == 'e') || (c == 'E'))
        {Stuff_Character(c, GLOBAL_dataFile); Phase = 3;
        }
        else Finished = TRUE;
    break;
    ...
    case 3: if (isdigit(c) || (c == '+') || (c == '-'))
        {Stuff_Character(c, GLOBAL_dataFile); Phase = 4;
        }
        else Finished = TRUE;
    break;
...
if ((Symbol_Id = Find_Symbol()) == -1) {
    if (++Number_Of_Symbols < Max_Symbols) {
        if ((Symbol_Table[Number_Of_Symbols] = new char[strlen(String)+1]) == NULL)
            Token_Error (GLOBAL_dataFile, "Out of memory. Cannot allocate space for identifier");
...
if(Case_Sensitive_Flag==0){
    if (strcmp (GLOBAL_reservedWords[i].Token_Name, &(String[0])) == 0)
     ...
     if (pov_stricmp ((char *)GLOBAL_reservedWords[i].Token_Name, &(String[0])) == 0)
     return (GLOBAL_reservedWords[i].Token_Number);
} 
...
{
GLOBAL_token.Identifier_Number = (int) GLOBAL_token.Token_Id - (int) LAST_TOKEN;
...
{
fprintf(stderr, "Error in %s line %d\n", GLOBAL_dataFile->Filename, GLOBAL_dataFile->Line_Number);
fputs(str, stderr);
fputs("\n\n", stderr);
exit(1);
}
