...
static void Link_Shapes(Light *New_Object,Light **Field,Light **Old_Object_List);
...
RGBAColorPaletteSpan *Construction_Map = NULL; /* moved here to allow reinitialization */
...
while (!Exit_Flag) {Get_Token();  switch (GLOBAL_token.Token_Id) {
...
#define END_EXPECT } } }
#define GET(x) Get_Token(); if (GLOBAL_token.Token_Id != x) Parse_Error (x)
#define UNGET Unget_Token();


/* Parse the file into the given frame. */
void Parse(Frame *Frame_Ptr)
{
     SimpleBody *Object;
    ...
    CASE (IDENTIFIER_TOKEN)
        if ((Constant_Id = Find_Constant()) != -1)
            if (Constants[(int)Constant_Id].Constant_Type == VECTOR_CONSTANT)
                *Given_Vector = *((Vector3D *) Constants[(int)Constant_Id].Constant_Data);
            else
                Type_Error ();
        else
            Undeclared ();
...
void Parse_Colour(RGBAColor *Given_Colour)
{
    CONSTANT Constant_Id;
    ...
    CASE (IDENTIFIER_TOKEN)
        if ((Constant_Id = Find_Constant()) != -1)
            if (Constants[(int)Constant_Id].Constant_Type == COLOUR_CONSTANT)
                *Given_Colour = *((RGBAColor *) Constants[(int)Constant_Id].Constant_Data);
            else
                Type_Error ();
        else
            Undeclared ();
...
RGBAColorPalette *Parse_Colour_Map ()
{
    ...
    CASE (IDENTIFIER_TOKEN)
        if ((Constant_Id = Find_Constant()) != -1)
            if (Constants[(int)Constant_Id].Constant_Type == LIGHT_SOURCE_CONSTANT)
                Local_Shape = (Light *)Copy((SimpleBody *) Constants[(int)Constant_Id].Constant_Data);
            else 
                Type_Error ();
        else
            Undeclared ();
    ...
    CASE (IDENTIFIER_TOKEN)
        if ((Constant_Id = Find_Constant()) != -1)
            if (Constants[(int)Constant_Id].Constant_Type == SPHERE_CONSTANT)
                Local_Shape = (Sphere *)Copy((SimpleBody *) Constants[(int)Constant_Id].Constant_Data);
            else
                Type_Error ();
        else
            Undeclared ();
    ...
    CASE (IDENTIFIER_TOKEN)
        if ((Constant_Id = Find_Constant()) != -1)
            if (Constants[(int)Constant_Id].Constant_Type == PLANE_CONSTANT)
                Local_Shape = (InfinitePlane *)Copy((SimpleBody *) Constants[(int)Constant_Id].Constant_Data);
            else
                Type_Error ();
        else
            Undeclared ();
    ...
    CASE (IDENTIFIER_TOKEN)
        if ((Constant_Id = Find_Constant()) != -1)
            if (Constants[(int)Constant_Id].Constant_Type == HEIGHT_FIELD_CONSTANT)
                Local_Shape = (HeightField *)Copy((SimpleBody *)Constants[(int)Constant_Id].Constant_Data);
            else
                Type_Error ();
        else
            Undeclared ();
    ...
    CASE (IDENTIFIER_TOKEN)
        if ((Constant_Id = Find_Constant()) != -1)
            if (Constants[(int)Constant_Id].Constant_Type == TRIANGLE_CONSTANT)
                Local_Shape = (Triangle *)Copy((SimpleBody *) Constants[(int)Constant_Id].Constant_Data);
            else
                Type_Error ();
        else
            Undeclared ();
    ...
    CASE (IDENTIFIER_TOKEN)
        if ((Constant_Id = Find_Constant()) != -1)
            if (Constants[(int)Constant_Id].Constant_Type == SMOOTH_TRIANGLE_CONSTANT)
                Local_Shape = (SmoothTriangle *)Copy((SimpleBody *) Constants[(int)Constant_Id].Constant_Data);
            else
                Type_Error ();
        else
            Undeclared ();
    ...
    CASE (IDENTIFIER_TOKEN)
        if ((Constant_Id = Find_Constant()) != -1)
            if (Constants[(int)Constant_Id].Constant_Type == QUADRIC_CONSTANT)
                Local_Shape = (Quadric *)Copy((SimpleBody *) Constants[(int)Constant_Id].Constant_Data);
            else
                Type_Error ();
        else
            Undeclared ();
    ...
    CASE (IDENTIFIER_TOKEN)
        if ((Constant_Id = Find_Constant()) != -1)
            if (Constants[(int)Constant_Id].Constant_Type == POLY_CONSTANT)
                Local_Shape = (Poly *)Copy((SimpleBody *) Constants[(int)Constant_Id].Constant_Data);
            else
                Type_Error ();
        else
            Undeclared ();
    ...
    CASE (IDENTIFIER_TOKEN)
        if ((Constant_Id = Find_Constant()) != -1)
            if (Constants[(int)Constant_Id].Constant_Type==BICUBIC_PATCH_CONSTANT)
                Local_Shape = (BicubicPatch *) Copy((SimpleBody *) Constants[(int)Constant_Id].Constant_Data);
            else
                Type_Error ();
        else
            Undeclared ();
    ...
    CASE (IDENTIFIER_TOKEN)
        if ((Constant_Id = Find_Constant()) != -1)
            if (Constants[(int)Constant_Id].Constant_Type == BOX_CONSTANT)
                Local_Shape = (Box *)Copy((SimpleBody *) Constants[(int)Constant_Id].Constant_Data);
            else
                Type_Error ();
        else
            Undeclared ();
...
Geometry *Parse_Blob ()
{
    ...
    MakeBlob((SimpleBody *)Local_Shape, threshold, blob_components, npoints, 0);
    EXIT
    END_CASE
    ...
    CASE (IDENTIFIER_TOKEN)
        if ((Constant_Id = Find_Constant()) != -1)
            if (Constants[(int)Constant_Id].Constant_Type == BLOB_CONSTANT)
                Local_Shape = (Blob *)Copy((SimpleBody *) Constants[(int)Constant_Id].Constant_Data);
            else
                Type_Error ();
        else
            Undeclared ();
    ...
    CASE (IDENTIFIER_TOKEN)
        if ((Constant_Id = Find_Constant()) != -1)
            if ((Constants[(int)Constant_Id].Constant_Type == CSG_INTERSECTION_CONSTANT)
                ...
                delete Container;
                Container = (CSG *) Copy ((SimpleBody *) Constants[(int)Constant_Id].Constant_Data);
...
Parse_Shape(SimpleBody *Object)
{
    ...
    CASE (UNION_TOKEN)
        Local_Shape = (Geometry *) Parse_CSG (CSG_UNION_TYPE, Object);
    EXIT
    END_CASE

    CASE (INTERSECTION_TOKEN)
        Local_Shape = (Geometry *) Parse_CSG (CSG_INTERSECTION_TYPE, Object);
    EXIT
    END_CASE

    CASE (DIFFERENCE_TOKEN)
        Local_Shape = (Geometry *) Parse_CSG (CSG_DIFFERENCE_TYPE, Object);
    EXIT
    END_CASE
...
SimpleBody *Parse_Object ()
{
    ...
    CASE (IDENTIFIER_TOKEN)
        if ((Constant_Id = Find_Constant()) != -1)
            if (Constants[(int)Constant_Id].Constant_Type == OBJECT_CONSTANT)
                Object = (SimpleBody *) Copy((SimpleBody *) Constants[(int)Constant_Id].Constant_Data);
            else
                Type_Error ();
        else
            Undeclared ();
    ...
    CASE (RIGHT_CURLY_TOKEN)
        ...
        UNGET
        Local_Shape = Parse_Shape(Object);
    Link((SimpleBody *) Local_Shape,
        (SimpleBody **) &(Local_Shape -> Next_Object),
...
Parse_Composite()
{
    ...
    CASE (IDENTIFIER_TOKEN)
        if ((Constant_Id = Find_Constant()) != -1)
            if (Constants[(int)Constant_Id].Constant_Type == COMPOSITE_CONSTANT)
                Local_Composite = (Composite *)Copy((SimpleBody *) Constants[(int)Constant_Id].Constant_Data);
            else             
                Type_Error ();
        else
            Undeclared ();
    ...
    Link((SimpleBody *) Local_Object,(SimpleBody **) &(Local_Object -> Next_Object),
    ...
    CASE (RIGHT_CURLY_TOKEN)
        ...
        OTHERWISE
        UNGET
        Local_Shape = Parse_Shape((SimpleBody *) Local_Composite);
    Link((SimpleBody *) Local_Shape,
        (SimpleBody **) &(Local_Shape -> Next_Object),
...
Post_Process_Shape(Geometry *Shape)
{
    ...
    if ((Shape->Type == CSG_UNION_TYPE) 
        ...
        for (Temp_Shape = ((CSG *)Shape)->Shapes;
                Temp_Shape != NULL ; 
