...
{
    const char *file = (ctx.token().getFileName() != nullptr) ? ctx.token().getFileName() : "<unknown>";
    const int line = ctx.token().getTokenLineNumber() + 1;
    const int column = (ctx.token().getTokenColumnNumber() > 0) ? ctx.token().getTokenColumnNumber() : 1;
    {
        char _logMsg[1024];
...
ParseErrorReporter::parseError(TOKEN tokenId, ParserContext &ctx)
{
    ...
    if (ctx.getReportingConfig() != nullptr &&
        ParseReportingConfiguration::writesVerboseErrors(*ctx.getReportingConfig())) {
        statFile = fopen(ParseReportingConfiguration::statFileName(*ctx.getReportingConfig()), "w+t");
