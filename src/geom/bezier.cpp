...
static void bezier_value(Vector3D *Result, DBL u, DBL v,
     Vector3D (*Control_Points)[4][4]);
static void bezier_partial(Vector3D *Result, DBL u, DBL v,
     BicubicPatch *Shape);
static int subpatch_normal (Vector3D *v1, Vector3D *v2, Vector3D *v3,
     Vector3D *Result, DBL *d);
static int intersect_subpatch (int Patch_Type, Ray *ray,
     Vector3D *v1, Vector3D *v2, Vector3D *v3,
     Vector3D *n, DBL d,
     Vector3D *n1, Vector3D *n2, Vector3D *n3,
     DBL *Depth, Vector3D *IP, Vector3D *IP_Norm);
static void find_average(int vector_count, Vector3D *vectors,
     Vector3D *center, DBL *radius);
static int spherical_bounds_check(Ray *ray, Vector3D *center,
     DBL radius);
static int intersect_bicubic_patch0(Ray *ray, BicubicPatch *Shape,
     DBL *Depths);
static int intersect_bicubic_patch1(Ray *ray, BicubicPatch *Shape,
     DBL *Depths);
static int intersect_bicubic_patch2(Ray *ray, BicubicPatch *Shape,
     DBL *Depths);
static int intersect_bicubic_patch3(Ray *ray, BicubicPatch *Shape,
     DBL *Depths);
static int intersect_bicubic_patch4(Ray *ray, BicubicPatch *Shape,
     DBL *Depths);
...
static void bezier_subpatch_intersect(Ray *, BicubicPatch *,
     Vector3D (*)[4][4], DBL, DBL, DBL,
     int, int *, DBL *, DBL *, DBL *);
static void bezier_split_left_right(Vector3D (*)[4][4],Vector3D (*)[4][4],
     Vector3D (*)[4][4]);
static void bezier_split_up_down(Vector3D (*)[4][4], Vector3D (*)[4][4],
     Vector3D (*)[4][4]);
static void bezier_subdivider(Ray *, BicubicPatch *,Vector3D (*)[4][4],
...
static void bezier_tree_walker(Ray *, BicubicPatch *, BezierNode *,
     int, int *, DBL *);
...
bezier_tree_builder(BicubicPatch *Object, Vector3D (*Patch)[4][4], int depth)
{
    ...
    if (flat_enough(Object, Patch)) {
        ...
        if (depth >= Object->V_Steps) {
            ...
            bezier_split_up_down(Patch, (Vector3D (*)[4][4])Lower_Left, (Vector3D (*)[4][4])Upper_Left);
            Node->Node_Type = BEZIER_INTERIOR_NODE;
            Children = create_bezier_child_block();
            Children->Children[0]=bezier_tree_builder(Object,(Vector3D (*)[4][4])Lower_Left,depth+1);
            Children->Children[1]=bezier_tree_builder(Object,(Vector3D (*)[4][4])Upper_Left,depth+1);
            Node->Count = 2;
            Node->Data_Ptr = (void *)Children;
        }
    else if (depth >= Object->V_Steps) {
        bezier_split_left_right(Patch, (Vector3D (*)[4][4])Lower_Left, (Vector3D (*)[4][4])Lower_Right);
        Node->Node_Type = BEZIER_INTERIOR_NODE;
        Children = create_bezier_child_block();
        Children->Children[0] = bezier_tree_builder(Object,(Vector3D (*)[4][4])Lower_Left,depth+1);
        Children->Children[1] = bezier_tree_builder(Object,(Vector3D (*)[4][4])Lower_Right,depth+1);
        Node->Count = 2;
        Node->Data_Ptr = (void *)Children;
    }
    else {
        bezier_split_left_right(Patch, (Vector3D (*)[4][4])Lower_Left, (Vector3D (*)[4][4])Lower_Right);
        bezier_split_up_down((Vector3D (*)[4][4])Lower_Left, (Vector3D (*)[4][4])Lower_Left, (Vector3D (*)[4][4])Upper_Left);
        bezier_split_up_down((Vector3D (*)[4][4])Lower_Right, (Vector3D (*)[4][4])Lower_Right, (Vector3D (*)[4][4])Upper_Right);
        Node->Node_Type = BEZIER_INTERIOR_NODE;
        Children = create_bezier_child_block();
        Children->Children[0] = bezier_tree_builder(Object,(Vector3D (*)[4][4])Lower_Left,depth+1);
        Children->Children[1] = bezier_tree_builder(Object,(Vector3D (*)[4][4])Upper_Left,depth+1);
        Children->Children[2] = bezier_tree_builder(Object,(Vector3D (*)[4][4])Lower_Right,depth+1);
        Children->Children[3] = bezier_tree_builder(Object,(Vector3D (*)[4][4])Upper_Right,depth+1);
        Node->Count = 4;
        Node->Data_Ptr = (void *)Children;
    }
    return Node;
...
static int
subpatch_normal(Vector3D *v1, Vector3D *v2, Vector3D *v3, Vector3D *Result, DBL *d)
{
    Vector3D V1, V2;
    DBL Length;
...
static int
intersect_subpatch(int Patch_Type, Ray *ray,
     Vector3D *v1, Vector3D *v2, Vector3D *v3,
     Vector3D *n, DBL d, Vector3D *n1, Vector3D *n2, Vector3D *n3,
     DBL *Depth, Vector3D *IP, Vector3D *IP_Norm)
{
...
Precompute_Patch_Values(BicubicPatch *Shape)
{
    ...
    if (Shape->Interpolated_Grid == NULL) {
        ...
        for (i=0;i<=Shape->U_Steps;i++) {
            Shape->Interpolated_Normals[i] = new Vector3D[2*(Shape->V_Steps+1)];
            if (Shape->Interpolated_Normals == NULL)
...
bezier_subpatch_intersect(Ray *ray, BicubicPatch *Shape,
     Vector3D (*Patch)[4][4], DBL u0, DBL u1, DBL v0,
     int recursion_depth, int *depth_count,
...
static void
bezier_split_up_down(Vector3D (*Patch)[4][4],
     Vector3D (*Top_Patch)[4][4], Vector3D (*Bottom_Patch)[4][4])
{
    Vector3D Temp1[4], Temp2[4], Half;
    int i, j;
     ...
     DBL *Depths, DBL *U_Values, DBL *V_Values)
{
    ...
    if (recursion_depth >= Object->U_Steps)
        if (recursion_depth >= Object->V_Steps)
            ...
            bezier_split_up_down(Patch, (Vector3D (*)[4][4])Lower_Left, (Vector3D (*)[4][4])Upper_Left);
            vt = (v1 - v0) / 2.0;
            bezier_subdivider(ray, Object, (Vector3D (*)[4][4])Lower_Left, u0, u1, v0, vt,
                recursion_depth+1, depth_count, Depths,
                    U_Values, V_Values);
            bezier_subdivider(ray, Object, (Vector3D (*)[4][4])Upper_Left, u0, u1, vt, v1,
                recursion_depth+1, depth_count, Depths,
                U_Values, V_Values);
        }
        ...
        bezier_split_left_right(Patch, (Vector3D (*)[4][4])Lower_Left, (Vector3D (*)[4][4])Lower_Right);
        ut = (u1 - u0) / 2.0;
        bezier_subdivider(ray, Object, (Vector3D (*)[4][4])Lower_Left, u0, ut, v0, v1,
            recursion_depth+1, depth_count, Depths,
            U_Values, V_Values);
        bezier_subdivider(ray, Object, (Vector3D (*)[4][4])Lower_Right, ut, u1, v0, v1,
            recursion_depth+1, depth_count, Depths,
            U_Values, V_Values);
    }
    else {
        ...
        bezier_split_up_down((Vector3D (*)[4][4])Lower_Left, (Vector3D (*)[4][4])Lower_Left, (Vector3D (*)[4][4])Upper_Left);
        bezier_split_up_down((Vector3D (*)[4][4])Lower_Right, (Vector3D (*)[4][4])Lower_Right, (Vector3D (*)[4][4])Upper_Right);
        bezier_subdivider(ray, Object, (Vector3D (*)[4][4])Lower_Left, u0, ut, v0, vt,
            recursion_depth+1, depth_count, Depths,
                U_Values, V_Values);
        bezier_subdivider(ray, Object, (Vector3D (*)[4][4])Upper_Left, u0, ut, vt, v1,
            recursion_depth+1, depth_count, Depths,
            U_Values, V_Values);
        bezier_subdivider(ray, Object, (Vector3D (*)[4][4])Lower_Right, ut, u1, v0, vt,
            recursion_depth+1, depth_count, Depths,
            U_Values, V_Values);
        bezier_subdivider(ray, Object, (Vector3D (*)[4][4])Upper_Right, ut, u1, vt, v1,
            recursion_depth+1, depth_count, Depths,
            U_Values, V_Values);
    }
}
...
static void
bezier_tree_walker(Ray *ray, BicubicPatch *Shape, BezierNode *Node,
     int depth, int *depth_count, DBL *Depths)
{
    BezierChild *Children;
    BezierVertices *Vertices;
...
int
All_Bicubic_Patch_Intersections(SimpleBody *Object, Ray *ray, PriorityQueueNode *Depth_Queue)
{
...
void
Bicubic_Patch_Normal(Vector3D *Result, SimpleBody *Object, Vector3D *Intersection_Point)
{
...
void Invert_Bicubic_Patch(SimpleBody *Object)
{
    ;
}
