...
static int
Intersect_Pixel(int x, int z, Ray *Ray, HeightField *H_Field, DBL height1,
     DBL height2)
{
...
static int
Intersect_Hf_Node(Ray *Ray, HeightField *H_Field, Vector3D *start, Vector3D *end)
{
...
Find_Hf_Min_Max(HeightField *H_Field, RGBAImage *image, int Image_Type)
{
    ...
    for(i=0; i<w; i++) {
        H_Field->Block[i] = (HeightFieldBlock *)calloc(h,sizeof(HeightFieldBlock));
    ...
    for(j=0; j < h; j++){
        for(j2=0;(j2 <= n) && (j*n+j2 <= max_z);j2++){
            ...
            if((z >= 0) && (z < max_z) && (j2!=n)) {
                switch (Image_Type) {
                case GIF: 
                    delete image->data.map_lines[max_z - z - 1]; break;
                case POT: 
                    delete image->data.map_lines[max_z - z - 1]; break;
                case TGA:
...
int
All_HeightFld_Intersections(SimpleBody *Object, Ray *ray, PriorityQueueNode *Depth_Queue)
{
...
void
HeightFld_Normal(Vector3D *Result, SimpleBody *Object, Vector3D *Intersection_Point)
{
    HeightField *H_Field = (HeightField *) Object;
    int px,pz;
    DBL x,z,y1,y2,y3;
