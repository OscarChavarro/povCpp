...
main(int argc, char *argv[])
{
    ...
    if (Options & DISKWRITE) {
        ...
        case 't':
        case 'T':
            if ((GLOBAL_outputFileHandle = Get_Targa_File_Handle()) == NULL) {
                close_all();
                exit(1);
            }
            break;
        ...
        if (Output_File_Name[0] == '\0')
            strcpy (Output_File_Name, Default_File_Name (GLOBAL_outputFileHandle));
    }
...
Print_Options()
{
        ...
        fprintf (stdout,"-q%d -w%d -h%d -s%d -e%d\n-i%s ",
            Quality, GLOBAL_frame.Screen_Width, GLOBAL_frame.Screen_Height,
...
print_stats()
{
    ...
    Pixels_In_Image = (long)GLOBAL_frame.Screen_Width * (long)GLOBAL_frame.Screen_Height;
