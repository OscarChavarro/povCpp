...
static void do_light(Light *Light_Source, DBL *Light_Source_Depth,
     Ray *Light_Source_Ray, Vector3D *Intersection_Point, RGBAColor *Light_Colour);
static int do_blocking(Intersection *Local_Intersection, 
     RGBAColor *Light_Colour, PriorityQueueNode *Local_Queue);
static void do_phong(Texture *Texture, Ray *Light_Source_Ray,
     Vector3D Eye, Vector3D *Surface_Normal, RGBAColor *Colour, RGBAColor *Light_Colour,
     RGBAColor *Surface_Colour);
static void do_specular(Texture *Texture, Ray *Light_Source_Ray,
     Vector3D REye, Vector3D *Surface_Normal, RGBAColor *Colour, RGBAColor *Light_Colour,
     RGBAColor *Surface_Colour);
...
void
Diffuse(Texture *Texture, Vector3D *Intersection_Point, Ray *Eye, Vector3D *Surface_Normal, RGBAColor *Surface_Colour, RGBAColor *Colour, DBL Attenuation)
{
    ...
    for (Light_Source = GLOBAL_frame.Light_Sources ; 
          Light_Source != NULL;
        ...
        if (Quality > 3) {
            ...
            for (Blocking_Object = GLOBAL_frame.Objects ; 
                ...
                for (All_Intersections (Blocking_Object, &Light_Source_Ray, Local_Queue) ;
                                 (Local_Intersection = Local_Queue->getHighest()) != NULL ;
...
static void
do_light(Light *Light_Source, DBL *Light_Source_Depth,
     Ray *Light_Source_Ray, Vector3D *Intersection_Point, RGBAColor *Light_Colour)
{
    DBL Attenuation = 1.0;
...
static int
do_blocking(Intersection *Local_Intersection, RGBAColor *Light_Colour, PriorityQueueNode *Local_Queue)
{
    ...
    if (Surface_Normal == NULL) {
        ...
        (Colour -> Blue) += Temp_Colour.Blue;
            ...
            if (ray -> Containing_Index == -1)
                ...
                ior = (GLOBAL_frame.Atmosphere_IOR)/(Texture -> Object_Index_Of_Refraction);
            }
            else
            {
...
void
Determine_Surface_Colour(Intersection *Ray_Intersection, RGBAColor *Colour,
     Ray *ray, int Shadow_Ray)
{
        ...
        if (Shadow_Ray) {
            ...
            if (GLOBAL_frame.Fog_Distance != 0.0)  {
                Fog (Ray_Intersection->Depth, &GLOBAL_frame.Fog_Colour, GLOBAL_frame.Fog_Distance,
                Colour);
            }
        }
