...
Create_Ray(Ray *ray, int width, int height, DBL x, DBL y)
{
    ...
    Y_Scalar = (( (DBL)(GLOBAL_frame.Screen_Height - 1) - y) -
...
Read_Rendered_Part()
{
    ...
    while ((rc = Read_Line(GLOBAL_outputFileHandle, Previous_Line, &line_number)) == 1) {
        if (Options & DISPLAY)
        ...
        if (rc == 0) {
            ...
            if (Open_File (GLOBAL_outputFileHandle, Output_File_Name,
                &GLOBAL_frame.Screen_Width, &GLOBAL_frame.Screen_Height, File_Buffer_Size,
                APPEND_MODE) != 1) {
...
Start_Tracing()
{
        ...
        for (y = (Options & ANTIALIAS)?First_Line-1:First_Line; y<Last_Line; y++) {
            ...
            for (x = 0 ; x < GLOBAL_frame.Screen_Width ; x++) {
                ...
                Create_Ray (VP_Ray, GLOBAL_frame.Screen_Width, GLOBAL_frame.Screen_Height, (DBL) x, (DBL) y);
                Trace_Level = 0;
                Trace (&ray, &Colour);
                Clip_Colour (&Colour, &Colour);
...
check_stats(register int y)
{
        ...
        if (Options & VERBOSE && VerboseFormat=='0'){
            ...
            if((First_Line != 0) || (Last_Line != GLOBAL_frame.Screen_Height))
                ...
                printf (":\n");
            printf ("Res %4d X %4d. Calc line %4d of %4d",GLOBAL_frame.Screen_Width, GLOBAL_frame.Screen_Height, (y-First_Line)+1, Last_Line-First_Line);
            if (!(Options & ANTIALIAS))
                printf(".");
        }
        ...
        if (Options & VERBOSE && VerboseFormat=='1'){
            fprintf (stderr,"Res %4d X %4d. Calc line %4d of %4d",GLOBAL_frame.Screen_Width, GLOBAL_frame.Screen_Height, (y-First_Line)+1, Last_Line-First_Line);
            if (!(Options & ANTIALIAS))
                fprintf(stderr,".");
        }
...
do_anti_aliasing(register int x, register int y, RGBAColor *Colour)
{
            ...
            if (Colour_Distance (&Current_Line[x-1], &Current_Line[x])
                ...
                if (!(Current_Line_Antialiased_Flags[x-1])) {
                    Supersample (&Current_Line[x-1],
                        x-1, y, GLOBAL_frame.Screen_Width, GLOBAL_frame.Screen_Height);
            ...
            if (Colour_Distance (&Previous_Line[x], &Current_Line[x])
                ...
                if (!(Previous_Line_Antialiased_Flags[x])) {
                    Supersample (&Previous_Line[x],
                        x, y-1, GLOBAL_frame.Screen_Width, GLOBAL_frame.Screen_Height);
        ...
        if (Antialias_Center_Flag) {
            Supersample (&Current_Line[x],
                x, y, GLOBAL_frame.Screen_Width, GLOBAL_frame.Screen_Height);
...
Trace(Ray *Ray, RGBAColor *Colour)
{
    ...
    for (Object = GLOBAL_frame.Objects ;
                  Object != NULL ;
