 ...
 *    #define DBL_FORMAT_STRING "%lf"         - the string to use for scanfs of
 *doubles #define DEFAULT_OUTPUT_FORMAT  'd'     - the default +f format #define
 *RED_RAW_FILE_EXTENSION ".red" - for +fr, this is the extension for the red
 *file #define GREEN_RAW_FILE_EXTENSION ".grn"  - ditto for the green file
 *    #define BLUE_RAW_FILE_EXTENSION ".blu"    - ditto again for the blue file
 *    #define FILENAME_SEPARATOR "/"          - the character that separates
 *names in a path. #define CASE_SENSITIVE_DEFAULT 2      - controls case
 *sensitivity in DAT files #define READ_FILE_STRING "rb"          - special
 *binary (raw, uncooked) #define WRITE_FILE_STRING "wb"            modes of
 *fopen() #define APPEND_FILE_STRING "ab" #define NORMAL '0' - machine-specific
 *PaletteOption settings #define GREY    'G'
 *
 ...
 *
 *    #define STARTUP_POVRAY                 - first executable statement of
 *main (useful for initialization) #define PRINT_OTHER_CREDITS              -
 *used for people who extend the machine specific modules (put your own printf's
 *here) #define TEST_ABORT                          - code to test for ^C or
 *abort (called on each pixel) #define FINISH_POVRAY                  - last
 *statement before exiting normally #define COOPERATE                          -
 *called for multi-tasking
 *
 ...
 *    #define labs(x) (long) ((x<0)?-x:x)    - Returns the absolute value of
 *long integer x #define max (x,y) ((x<y)?y:x)            - greater of x and y
 *
 ...
 *setvbuf. If not, just define it away - it's not critical to the raytracer.  It
 *                                                         just buffers disk
 *accesses.
 *
 *    These allow command line vs non-command line versions and alternate
 *    main routines.  Normally they should be undefined.  Define them 1 if
