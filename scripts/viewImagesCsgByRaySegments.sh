...
OUTPUT_DIR="${OUTPUT_DIR:-outputCsgByRaySegments}"
...
work_dir="$(mktemp -d /tmp/viewImagesCsgByRaySegments.XXXXXX)"
...
process_image() {
  ...
  if [[ ! -f "${output_file}" ]]; then
    # outputCsgByRaySegments is filled incrementally (renderCsgRoth.sh may
    # only have been run for a subset of scenes), so a missing render here
    # is expected, not fatal: skip this one comparison (return 0, not a job
    # failure) and let every other background job keep building/showing its
    # own comparison image regardless.
